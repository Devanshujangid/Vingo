...
function UserDashboard() {
  ...
  const updateButton = (ref, setLeftButton, setRightButton) => {
    ...
    if (element) {
      setLeftButton(element.scrollLeft > 0);
      setRightButton(element.scrollLeft + element.clientWidth < element.scrollWidth - 1); // -1 for precision
    }
  };
  ...
  useEffect(() => {
    ...
    const handleCateScroll = () => updateButton(cateRef, setShowLeftCateButton, setShowRightCateButton);
    const handleShopScroll = () => updateButton(shopRef, setShowLeftShopButton, setShowRightShopButton);

    if (cateRef) {
      updateButton(cateRef, setShowLeftCateButton, setShowRightCateButton);
      cateRef.addEventListener('scroll', handleCateScroll);
    }
    if (shopRef) {
      updateButton(shopRef, setShowLeftShopButton, setShowRightShopButton);
  ...
  }, [categories, shopInMyCity, itemsInMyCity]); // Added dependencies to re-evaluate when content changes
        ...
        <div className='w-full relative'>
          ...
          <div className='w-full flex overflow-x-auto gap-4 pb-2 scrollbar-hide' ref={shopScrollRef}>
            {shopInMyCity?.map((shop) => (
              // ✅ Changed key from index to the unique shop._id
              <CategoryCard name={shop.name} image={shop.image} key={shop._id} onClick={() => navigate(`/shop/${shop._id}`)} />
            ))}
          </div>
          {showRightShopButton && <button className='absolute right-0 top-1/2 -translate-y-1/2 bg-[#ff4d2d] text-white p-2 rounded-full shadow-lg hover:bg-[#e64528] z-10' onClick={() => scrollHandler(shopScrollRef, "right")}>
            <FaCircleChevronRight />
